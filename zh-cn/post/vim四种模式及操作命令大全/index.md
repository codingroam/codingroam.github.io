...
- 在这个模式下：
  ...
  - 选中行，复制（ctrl+C）
  - 可以增、删
    - `x`删除光标后的一个字符，`nx（n是数字）`删除光标后的n个字符，`X`删除光标前的一个字符，
    - `dd`剪切光标所在的那一行，`ndd`剪切光标所在行后的n行
    - `p`光标所在行开始，向后粘贴已经复制的内容，`P`光标所在行开始，向前粘贴已经复制的内容
    - `yy`复制光标所在的行，`nyy`复制光标所在行后的n行
    - `u`还原上一次的操作
...
- 在该模式下可以进行保存，搜索，替换，退出，显示行号等。
  - `/word` 光标之后查找字符串word，按`n`向后搜索，按`N`向前搜索
  - `?word`光标之前查找字符串word，按`n`向前搜索，按`N`向前搜索
...
## [[常用] 剪切和复制、粘贴](http://www.cnblogs.com/jiqingwu/archive/2012/06/14/vim_notes.html#id71)
...
## [[常用]基本插入](http://www.cnblogs.com/jiqingwu/archive/2012/06/14/vim_notes.html#id68)
...
## [[常用] 基本移动](http://www.cnblogs.com/jiqingwu/archive/2012/06/14/vim_notes.html#id64)
...
![微博桌面截图_20150306094004](http://static.codeceo.com/images/2015/03/56ae8c4ac0da34ecbaf2b9737b0d47b8.jpg)
...
> 作者: wang  
> URL: https://codingroam.github.io/zh-cn/post/vim%E5%9B%9B%E7%A7%8D%E6%A8%A1%E5%BC%8F%E5%8F%8A%E6%93%8D%E4%BD%9C%E5%91%BD%E4%BB%A4%E5%A4%A7%E5%85%A8/  
